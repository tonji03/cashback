...
public class AcquistoDAOImpl implements AcquistoDAO{
    ...
    @Override
    public void insertIntoMovacqu(int id_cliente, double prezzo, Date data_acquisto) {
        String query = "INSERT INTO movacqu(id_cliente,prezzo,data_acquisto) values (?,?,?)";
    ...
    public List<Acquisto> getAllAcquisti(){
        ...
        String query ="select * from movacqu order by id_cliente, data_acquisto desc;";
        try(PreparedStatement st = connection.prepareStatement(query)){
            ResultSet rs = st.executeQuery();
            while (rs.next()){
    ...
    public List<Acquisto> getAllAcquistidd() {
        List<Acquisto> acquisti = new ArrayList<>();
        String query = "select * from movacqudd order by id_cliente, data_acquisto desc;";
        try(PreparedStatement st = connection.prepareStatement(query)){
            ResultSet rs = st.executeQuery();
            while (rs.next()){
                Acquisto acquisto = new Acquisto(rs.getInt("id_acquisto"),rs.getInt("id_cliente"), rs.getDouble("prezzo"), rs.getDate("data_acquisto"));
